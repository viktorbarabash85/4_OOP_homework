...
В этом домашнем задании работа продолжается в рамках того же проекта, 
который был обновлен ранее в `15_1_homework`. 
___
> Продолжаем работу над магазином: в этой домашней работе оптимизируем работу 
> с классами путем создания базовых классов и классов наследников.
...
>**Подсказка**
>
>Необходимо создать два класса, которые будут являться наследниками класса, 
> который создавали ранее для хранения продуктов. Важно обращать внимание на повторяющиеся свойства и методы, 
> чтобы их все вынести в класс-родитель и избежать повторения кода. При этом у продуктов должна оставаться категория, 
> в которой товары, не относящиеся к двум указанным классам, должны храниться как объект класса `Product`, 
> который мы реализовали ранее. Не забудьте проверить, что вся функциональность, реализованная ранее, 
> осталась работоспособной после внесения этих правок.
...
Доработайте функциональность сложения таким образом, чтобы можно было складывать товары 
...
>**Подсказка**
>
>Третье задание посвящено отработке использования функций `issubclass` и `isinstance`, 
> которые мы изучали в уроке. Они помогут добавить проверки при добавлении объектов в список продуктов.
